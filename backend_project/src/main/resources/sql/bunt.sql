...
DROP TABLE IF EXISTS `GAME_NUM_BASEBALL`;
CREATE TABLE `GAME_NUM_BASEBALL`
(
...
-- 외래 키 제약 조건 추가
ALTER TABLE `PLAYERS`
    ADD CONSTRAINT `FK_TEAM_TO_PLAYERS_1` FOREIGN KEY (`team_id`) REFERENCES `TEAM` (`team_id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `COMMUNITY_BOARD`
    ADD CONSTRAINT `FK_USERS_TO_COMMUNITY_BOARD_1` FOREIGN KEY (`user_id`) REFERENCES `USERS` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `COMMUNITY_BOARD_COMMENT`
    ADD CONSTRAINT `FK_COMMUNITY_BOARD_TO_COMMUNITY_BOARD_COMMENT_1` FOREIGN KEY (`community_board_id`) REFERENCES `COMMUNITY_BOARD` (`community_board_id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `COMMUNITY_BOARD_COMMENT`
    ADD CONSTRAINT `FK_USERS_TO_COMMUNITY_BOARD_COMMENT_1` FOREIGN KEY (`user_id`) REFERENCES `USERS` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
